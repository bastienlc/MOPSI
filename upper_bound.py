...
def get_upper_bound_with_milp(requests_list, rooms_list, max_iter=1):
    ...
    while total_capacity - len(attributions) > 0 and iteration < max_iter:
        ...
        selected_requests_dictonary = dictionary_from_requests(
            requests_list[: total_capacity + iteration]
        )  # requests are progressively added if all the rooms are not filled
        # process selected requests to remove inconsistent friend requests
        for request in selected_requests_dictonary.values():
            if (
                request.has_mate
                and not str(request.mate_id) in selected_requests_dictonary.keys()
    ...
    if len(attributions) < total_capacity:
        ...
        unassigned_selected_requests_selection = [
            True for _ in range(len(selected_requests_list))
        ]
        rooms_id_to_idx = {room.room_id: k for k, room in enumerate(rooms)}
        requests_id_to_idx = {
            request.student_id: k for k, request in enumerate(selected_requests_list)
        }
        for attribution in attributions:
            room = rooms[rooms_id_to_idx[attribution.room.room_id]]
            if room.capacity == len(room.students):
                unfilled_rooms_selection[
                    rooms_id_to_idx[attribution.room.room_id]
                ] = False
            unassigned_selected_requests_selection[
                requests_id_to_idx[attribution.request.student_id]
            ] = False
        unfilled_rooms = [
            room for k, room in enumerate(rooms) if unfilled_rooms_selection[k]
        ]
        unassigned_selected_requests_dictionary = {
            str(request.student_id): request
            for k, request in enumerate(selected_requests_list)
            if unassigned_selected_requests_selection[k]
        }
        for room in unfilled_rooms:  # Empty the half-filled rooms
            if room.students:
                ...
                for student in evicted_students:
                    unassigned_selected_requests_dictionary[
                        student
                    ] = selected_requests_list[requests_id_to_idx[student]]
        for request in unassigned_selected_requests_dictionary.values():
            if (
                request.has_mate
                and not str(request.mate_id)
                in unassigned_selected_requests_dictionary.keys()
            ):
                request.has_mate = False
                request.mate_id = None

        constraint_less_attributions = milp_solve(
            list(unassigned_selected_requests_dictionary.values()),
            unfilled_rooms,
            constraints=False,
        )
